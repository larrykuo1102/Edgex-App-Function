...
func (app *myApp) CreateAndRunAppService(serviceKey string, newServiceFactory func(string) (interfaces.ApplicationService, bool)) int {
	...
	if err := app.service.LoadCustomConfig(app.serviceConfig, CustomConfigSectionName); err != nil {
	...
	if err := app.service.ListenForCustomConfigChanges(&app.serviceConfig.AppCustom, CustomConfigSectionName, app.ProcessConfigUpdates); err != nil {
	...
	// See https://docs.edgexfoundry.org/latest/microservices/application/AdvancedTopics/#pipeline-per-topics for more details. SimulationServer/CounterNum
	err = app.service.AddFunctionsPipelineForTopics("Floats", []string{"events/device/SimulationServer/+/CounterNum/#"},
		sample.LogEventDetails,
		sample.ConvertEventToXML,
		sample.OutputXML)
	if err != nil {
